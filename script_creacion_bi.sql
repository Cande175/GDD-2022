...
BEGIN
	...
	SELECT DISTINCT YEAR(CARRERA_FECHA), MONTH(CARRERA_FECHA)
	FROM LOCRO.CARRERA
...
BEGIN
	...
	JOIN LOCRO.BI_TIEMPO
		ON YEAR(CARRERA_FECHA) = TIEMPO_ANIO AND MONTH(CARRERA_FECHA) = TIEMPO_CUATRIMESTRE
...
AS
	SELECT TOP 3
	(SELECT CIRCUITO_NOMBRE FROM LOCRO.BI_CIRCUITO c WHERE h.CIRCUITO_ID = c.CIRCUITO_ID) AS 'CIRCUITO',
